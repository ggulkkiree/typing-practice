...
if (studentData["1학년 1반"].length === 0) {
    studentData["1학년 1반"].push({
        name: "테스트학생",
        errorMode: "stop",
        allowedMenus: ["자리 연습", "낱말 연습", "문장 연습", "이력서 연습"],
        stats: { maxWpm: 0 },
        weakness: {}
    });
}
...
function showPwModal(title, action) {
    document.getElementById('pw-modal-title').innerText = title;
    document.getElementById('pw-modal-input').value = "";
    pwActionTarget = action;
...
document.getElementById('pw-modal-submit').addEventListener('click', () => {
    ...
    if (pwActionTarget === 'login') {
        if (pw === adminPassword) {
            closeModals(); showScreen('admin-screen'); renderStudentTable();
        } else { showAlert("비밀번호가 틀렸습니다."); }
    } else if (pwActionTarget === 'change') {
        if (pw.trim() !== "") {
            adminPassword = pw; safeSetItem('adminPw', adminPassword); showAlert("비밀번호가 성공적으로 변경되었습니다.");
        } else { showAlert("비밀번호를 입력해주세요."); }
    }
});

document.getElementById('pw-modal-input').addEventListener('keypress', (e) => {
    if (e.key === 'Enter') document.getElementById('pw-modal-submit').click();
});
...
document.getElementById('add-student-btn').addEventListener('click', () => {
    ...
    if (newName) {
        studentData[selectedClass].push({
            name: newName, errorMode: "stop", allowedMenus: ["자리 연습", "낱말 연습"],
            stats: { maxWpm: 0 }, weakness: {}
        });
        saveData(); document.getElementById('admin-new-student').value = '';
    ...
    studentData[selectedClass].forEach((student, index) => {
        ...
        let checkboxes = menus.map(menu => `
            <label class="checkbox-label">
                <input type="checkbox" onchange="updateMenu('${selectedClass}', ${index}, '${menu}', this.checked)" 
                ${student.allowedMenus.includes(menu) ? 'checked' : ''}> ${menu}
            </label>
        `).join('');

        let weakStr = "-";
        let diagnosis = "아직 데이터가 부족합니다.";
        ...
        if (student.weakness && Object.keys(student.weakness).length > 0) {
            ...
            if (totalErrors > 5) {
                if (leftErrors > rightErrors * 1.5) diagnosis = "왼손(자음) 소근육 훈련이 더 필요합니다.";
                else if (rightErrors > leftErrors * 1.5) diagnosis = "오른손(모음) 소근육 및 위치 인지가 약합니다.";
                else if (shiftErrors > totalErrors * 0.3) diagnosis = "쌍자음(Shift) 동시 조작 타이밍이 어렵습니다.";
                else diagnosis = "전반적인 위치 인지 훈련이 필요합니다.";
            } else {
                diagnosis = "원활하게 훈련 중입니다.";
            }
        }
        ...
        tr.innerHTML = `
            ...
            <td>
                <select onchange="updateErrorMode('${selectedClass}', ${index}, this.value)">
            ...
            <td><button class="delete-btn" onclick="deleteStudent('${selectedClass}', ${index})">삭제</button></td>
        `;
        tbody.appendChild(tr);
    });
}

window.updateErrorMode = (cls, idx, val) => { studentData[cls][idx].errorMode = val; saveData(); };
window.updateMenu = (cls, idx, menu, isChecked) => {
    let menus = studentData[cls][idx].allowedMenus;
    if (isChecked) menus.push(menu); else studentData[cls][idx].allowedMenus = menus.filter(m => m !== menu);
    saveData();
};
window.deleteStudent = (cls, idx) => { studentData[cls].splice(idx, 1); saveData(); renderStudentTable(); updateStudentSelects(); };
document.getElementById('admin-class').addEventListener('change', renderStudentTable);

function saveData() { safeSetItem('studentData', JSON.stringify(studentData)); }
...
loginBtn.addEventListener('click', () => {
    ...
    currentUser.allowedMenus.forEach(menu => {
        ...
        btn.innerText = menu; btn.onclick = () => startPractice(menu);
...
let totalAccumulatedStrokes = 0;
let totalErrorCount = 0; 
let currentCombo = 0;   
let requireEnter = false; 
let lastValidValue = "";
...
document.getElementById('practice-screen').addEventListener('click', (e) => {
    if(e.target.id !== 'end-practice-btn') typingInput.focus();
});

function updateTargetTextDisplay(inputVal) {
    let validLen = inputVal.length;
    let html = "";
...
function updateKeyboardGuide(inputVal) {
    ...
    if (isErrorState) {
        let bsKey = document.querySelector('.key[data-key="backspace"]');
        if (bsKey) bsKey.classList.add('active-error'); return;
    }

    if (requireEnter && inputVal === currentWord) {
        let enterKey = document.querySelector('.key[data-key="enter"]');
        if (enterKey) enterKey.classList.add('active'); return;
    }

    let inputJaso = Hangul.disassemble(inputVal);
    let targetJaso = Hangul.disassemble(currentWord);
    
    if (inputJaso.length < targetJaso.length) {
        let nextJaso = targetJaso[inputJaso.length];
        let keyCharCode = jasoToKeyMap[nextJaso];
        
        if (keyCharCode) {
            let keyEl = document.querySelector(`.key[data-key="${keyCharCode}"]`);
            if (keyEl) keyEl.classList.add('active');
            
            if (requiresShift.includes(nextJaso)) {
                let shiftKey = document.querySelector('.key[data-key="shift"]');
...
function startPractice(menuName) {
    currentMode = menuName;
    ...
    practiceQueue = [];
    for(let i = 0; i < WORDS_PER_SESSION; i++) practiceQueue.push(currentWordList[Math.floor(Math.random() * currentWordList.length)]);
    ...
    isPracticing = true; activeTimeMs = 0; wordStartTime = 0; 
...
function clearIME() {
    ...
    hidden.focus(); 
    setTimeout(() => { typingInput.value = ''; typingInput.focus(); }, 10);
}
...
function nextWord() {
    ...
    clearIME(); 
    lastValidValue = ""; isErrorState = false; wordStartTime = 0; 
...
typingInput.addEventListener('keydown', function(e) {
    ...
    if (currentUser.errorMode === 'stop' && isErrorState) {
        if (e.key !== 'Backspace' && e.key !== 'ArrowLeft' && e.key !== 'ArrowRight') {
            e.preventDefault(); this.classList.add('shake');
            setTimeout(() => this.classList.remove('shake'), 200); return;
        }
    }
...
typingInput.addEventListener('input', function(e) {
    ...
    if (inputVal.length === 0) {
        lastValidValue = ""; isErrorState = false; updateTargetTextDisplay("");
        this.classList.remove('text-error', 'shake');
        feedbackMsg.style.color = 'black'; feedbackMsg.innerText = "타자를 시작하세요."; return;
    }
    ...
    for (let i = 0; i < inputJaso.length; i++) {
        if (inputJaso[i] !== targetJaso[i]) { isError = true; mismatchIndex = i; break; }
    }
